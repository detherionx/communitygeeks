...
**How it works**: each badge is a fixed-size box (120×56 in the marquee, 96×40 in Selected Work) with two overlaid layers — `.logo-mono` (the logo rendered via CSS `mask-image`, filled with `var(--ink-soft)`, so it shows as a flat silhouette *regardless of the source image's actual colors*) and `.logo-color` (the same image via `background-image`, real colors, `opacity:0` by default). On `:hover`, the badge background flips to `var(--ink)` and the two layers cross-fade. The logo URL is passed once per instance via an inline `--logo-url` custom property, not duplicated in CSS.

**Why this solves the Space Whale / ZAM problem without touching their artwork**: both have white-only official marks (their sites are dark-background, no light variant exists). `mask-image` only cares about the source image's alpha channel/silhouette, not its actual painted color — so their white shapes work perfectly as a mask filled with our own ink color. On hover, the dark badge background is what makes their real white pixels visible instead of vanishing against a light background. This is why `clients.json`'s `selectedWork` entry for Space Whale explicitly warns not to swap its `logoPath` back into a plain `<img>` — it only works through this masked component.
